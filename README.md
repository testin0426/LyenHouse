...
收集参考视频、VRChat 参考，探索上限。

- 雨痕玻璃 [完成]
- 镜子交互测试: UI 只有开/关 [无。有思路，C# 与客户端 Rt 通信较麻烦]
- 相机交互测试 [半完成，有些问题]

总结：完成度偏低，钢琴靠近发声在录制时丢失，原因待查。
...
总结:
1. UI 交互问题需优先解决，否则会拖慢整体进度。需求如上。
2. 贴图需找人用 SP 绘制（数量视家具量而定）。要求会用 SP、看懂 shader、有材质选择能力（需了解 AO、GBuffer 等 RGB 合并相关），能自行在 Unity 中调试。
...
问题: 空间划分与钢琴尺寸仍对不上（钢琴尚未放入 DCC；设计图尺寸因引擎内调整地编而不准确）。
...
挑选可交互物品时可用 AI 工具快速添加功能，但资产导入导出需筛选模型、优化调整及 shader 改写，这部分计划外包。

...
namespace MFPP
{
    ...
    public class Player : MonoBehaviour
    {
        ...
        [SerializeField]
        private CameraSettings cameraExtraSettings;
        /// <summary>
        /// Extra camera settings.
        /// </summary>
        public CameraSettings CameraExtras
        {
            get { return cameraExtraSettings; }
            set { cameraExtraSettings = value; }
        }
        [SerializeField]
        ...
        public bool IsJumping
        {
            get
            {
                ...
                if (Movement.Jump.AllowJump)
                {
                    inputJump = Movement.Jump.AutoJump
                        ? GetButton(Controls.JumpButton)
                        : GetButtonDown(Controls.JumpButton);
                }

                if (inputJump)
                    Jumped = true;

                return Movement.AllowMovement && Movement.Jump.AllowJump && canJumpOnThisSlope && !ceilingRaycast && inputJump;
            }
        }
        ...
        public bool IsCrouching
        {
            get
            {
                ...
                if (Movement.Crouch.AllowCrouch)
                {
                    switch (Movement.Crouch.CrouchToggle)
                    {
                        case MovementSettings.CrouchSettings.CrouchToggleOption.CrouchHoldOnly:
                            wantsCrouch = GetButton(Controls.CrouchButton);
                            break;
                        case MovementSettings.CrouchSettings.CrouchToggleOption.CrouchToggleOnly:
                            wantsCrouch = crouchToggle;
                            break;
                        case MovementSettings.CrouchSettings.CrouchToggleOption.Both:
                            if (Controls.CrouchButton != Controls.CrouchToggleButton)
                                wantsCrouch = crouchToggle || GetButton(Controls.CrouchButton);
                            else
                                wantsCrouch = crouchToggle;
                            break;
                    }
                ...
                return Movement.Crouch.AllowCrouch && canCrouch && (Movement.AllowMovement && (wantsCrouch) || raycast);
            }
        }
        /// <summary>
        ...
        public Player()
        {
            ...
            CameraExtras = new CameraSettings(this);
        ...
        private void Start()
        {
            ...
            TargetLookAngles = new Vector2(transform.eulerAngles.y, Main.Camera.transform.localEulerAngles.x > 180 ? -Main.Camera.transform.localEulerAngles.x+360 : -Main.Camera.transform.localEulerAngles.x);
        ...
        protected void DoMouse()
        {
            ...
            {
                Vector2 rawDelta = new Vector2(GetCurrentAxis(Controls.MouseXAxis) * Controls.MouseSensitivityXMultiplier, GetCurrentAxis(Controls.MouseYAxis) * Controls.MouseSensitivityYMultiplier) * Controls.MouseSensitivity; // Raw delta of the mouse.
        ...
        protected void DoMovement()
        {
            ...
            else // Not grounded, in the air
            {
                ...
                if (JustAired && !IsJumping && !Jumped && KinematicMovement.sqrMagnitude <= 0 && OldKinematicMovement.sqrMagnitude <= 0)
                {
                    Vector3 v = new Vector3(FinalMovement.x, Physics.gravity.y * Time.deltaTime, FinalMovement.z);
                    FinalMovement = v;
                }
            ...
            if (Collisions.ContinuousCollisionDetection && Time.deltaTime > 0f) // Apply continuous collision detection
            {
        ...
        protected void UpdateHeight()
        {
            ...
            if (Main.Camera != null)
            {
                Main.Camera.transform.localPosition = (Vector3.up * CurrentCameraHeight) + (Vector3.forward * (CameraExtras.EnableCameraPitchOffset ? CameraExtras.PitchOffsetCurve.Evaluate(LookAngles.y) : 0f)); // Set the local position of the camera to the current camera height in the Y axis
            }
        }

        protected void DoMisc()
        {
            if (Movement.Crouch.CrouchToggle != MovementSettings.CrouchSettings.CrouchToggleOption.CrouchHoldOnly && Input.GetButtonDown(Controls.CrouchToggleButton))
        ...
        {
            public CameraSettings(Player player) : base(player) { }

            [SerializeField]
            [Tooltip("Determines if the forward offset by pitch of the camera is enabled.")]
            private bool enableCameraPitchOffset = false;
            /// <summary>
            /// Determines if the forward offset by pitch of the camera is enabled.
            /// </summary>
            public bool EnableCameraPitchOffset
            {
                get { return enableCameraPitchOffset; }
                set { enableCameraPitchOffset = value; }
            }

            [SerializeField]
            [Tooltip("The pitch offset curve, time dictates the pitch, value dictates the forward offset.")]
            private AnimationCurve pitchOffsetCurve = new AnimationCurve(new Keyframe(0f, 0f), new Keyframe(-90f, 0.2f));
            /// <summary>
            /// The pitch offset curve, time dictates the pitch, value dictates the forward offset.
            /// </summary>
            public AnimationCurve PitchOffsetCurve
            {
                get { return pitchOffsetCurve; }
                set { pitchOffsetCurve = value; }
            }
        }
        ...
        public class ControlSettings : Settings
        {
            ...
            [SerializeField]
            [Tooltip("The X axis multiplier of the sensitivity of the mouse.")]
            private float mouseSensitivityXMultiplier = 1f;
            /// <summary>
            /// The X axis multiplier of the sensitivity of the mouse.
            /// </summary>
            public float MouseSensitivityXMultiplier
            {
                get { return mouseSensitivityXMultiplier; }
                set { mouseSensitivityXMultiplier = value; }
            }
            [SerializeField]
            [Tooltip("The Y axis multiplier of the sensitivity of the mouse.")]
            private float mouseSensitivityYMultiplier = 1f;
            /// <summary>
            /// The Y axis multiplier of the sensitivity of the mouse.
            /// </summary>
            public float MouseSensitivityYMultiplier
            {
                get { return mouseSensitivityYMultiplier; }
                set { mouseSensitivityYMultiplier = value; }
            }
            [SerializeField]
            [Range(0f, 1f)]
            ...
            [SerializeField] 
            [Tooltip("Button name for the Crouch button (in toggle mode).")]
            private string crouchToggleButton = "Crouch Toggle";
            /// <summary>
            /// Button name for the Crouch button (in toggle mode).
            /// </summary>
            public string CrouchToggleButton
            {
                get { return crouchToggleButton; }
                set { crouchToggleButton = value; }
            }
        }
        ...
        public class MovementSettings : Settings
        {
            ...
            public class CrouchSettings : Settings
            {
                ...
                [SerializeField]
                [Tooltip("Crouch toggling mode (by default Crouch Hold Only).")]
                private CrouchToggleOption crouchToggle = CrouchToggleOption.CrouchHoldOnly;
                /// <summary>
                /// Crouch toggling mode (by default Crouch Hold Only).
                /// </summary>
                public CrouchToggleOption CrouchToggle
                {
                    get { return crouchToggle; }
                    set { crouchToggle = value; }
                }
                [SerializeField, Space]
                ...
                public enum CrouchToggleOption : byte
                {
                    CrouchHoldOnly,
                    CrouchToggleOnly,
                    Both
